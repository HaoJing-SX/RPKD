...
from pcdet.models import build_network, model_fn_decorator, build_teacher_network, build_teacher_network2
...
def parse_config():
    ...
    parser.add_argument('--teacher_ckpt2', type=str, default=None, help='teacher checkpoint to start from')
    ...
    # for mmd
    if cfg.get('TEACHER_CKPT2', None):
        args.teacher_ckpt2 = cfg.TEACHER_CKPT2
...
def main():
    ...
    if cfg.get('KD', None) and cfg.KD.ENABLED:
        ...
        # for mmd
        if cfg.KD.get('MMD', None) and cfg.KD.MMD:
            teacher_model2 = build_teacher_network2(cfg, args, train_set, dist_train, logger)
            kd_utils.prepare_kd_modules(teacher_model2, model)
    else:
        teacher_model = None
        teacher_model2 = None
    ...
    # no KD --> rain_utils.py/train_model
    # KD --> train_kd_utils.py/train_model_kd
    if cfg.get('KD', None) and cfg.KD.ENABLED:
        if cfg.KD.get('MMD', None) and cfg.KD.MMD:
            train_model_kd(
                model,
