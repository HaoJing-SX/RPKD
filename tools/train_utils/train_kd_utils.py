...
def train_one_epoch(model, optimizer, train_loader, model_func, lr_scheduler, accumulated_iter, optim_cfg,
                    ...
                    teacher_model2=None, extra_optim=None, extra_lr_scheduler=None):
    if total_it_each_epoch == len(train_loader):
        dataloader_iter = iter(train_loader)
    # forward_func = getattr(kd_forwad, cfg.KD.get('FORWARD_FUNC', 'forward'))
    # use kd_forward.py
    forward_func = getattr(kd_forwad, 'forward')
    ...
    # for cur_it in range(1):
        ...
        loss, tb_dict, disp_dict = forward_func(
            model, teacher_model, batch, optimizer, extra_optim, optim_cfg, load_data_to_gpu, teacher_model2
        )
        # print('$$$$$$$$$$$$$$4', batch.keys())
        accumulated_iter += 1
...
def train_model_kd(model, optimizer, train_loader, model_func, lr_scheduler, optim_cfg,
                   ...
                   merge_all_iters_to_one_epoch=False, teacher_model=None, teacher_model2=None):
    ...
    if teacher_model is not None and cfg.KD.get('TEACHER_MODE', None):
        ...
        if teacher_model2 is not None:
            getattr(teacher_model2, cfg.KD.TEACHER_MODE)()
            # print("@@@@@@@@@@@@@@@@@@@@@@@@@@@@ done1") # done
