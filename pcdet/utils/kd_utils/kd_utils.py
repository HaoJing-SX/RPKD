...
def process_kd_config():
    ...
    if cfg.KD.get('MMD', False):
        cfg.MODEL_TEACHER2.KD = True

    # Only student model have KD_LOSS config
    if cfg.KD_LOSS.ENABLED:
        cfg.MODEL.KD_LOSS = cfg.KD_LOSS

    parse_key_list = ['LOGIT_KD', 'FEATURE_KD', 'LABEL_ASSIGN_KD', 'VFE_KD', 'ROI_KD', 'RP_KD']
    ...
    for key in parse_key_list:
        if cfg.KD.get(key, None) and cfg.KD[key].ENABLED:
            ...
            # for mmd
            if cfg.KD.get('MMD', False):
                cfg.MODEL_TEACHER2[key] = cfg.KD[key]
...
def put_teacher_prior_to_student(teacher_model, student_model, model_cfg):
    # print("@@@@@@@@@@@@@@@ teacher_model.kd_head.voxel_size:", teacher_model.kd_head.voxel_size)
    # print("@@@@@@@@@@@@@@@ student_model.kd_head.voxel_size_tea:", student_model.kd_head.voxel_size_tea)
