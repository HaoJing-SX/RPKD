...
def build_teacher_network(cfg, args, train_set, dist, logger):
    ...
    if args.teacher_ckpt is not None:
        ...
        # teacher model Loading teacher parameters  OK
...
def build_teacher_network2(cfg, args, train_set, dist, logger):
    teacher_model = build_network(model_cfg=cfg.MODEL_TEACHER2, num_class=len(cfg.CLASS_NAMES), dataset=train_set)
    teacher_model.cuda()
    ...
    if args.teacher_ckpt2 is not None:
        logger.info('----------- Loading teacher parameters2 -----------')
        # teacher model Loading teacher parameters  OK
        teacher_model.load_params_from_file(filename=args.teacher_ckpt2, to_cpu=dist, logger=logger)
