...
class PointHeadTemplate(nn.Module):
    ...
    def build_losses(self, losses_cfg):
        ...
        elif reg_loss_type == 'MSELoss':
            self.reg_loss_func = getattr(torch.nn, reg_loss_type)(reduction='none')
            # print('@@@@@@@@@@@@@@ self.reg_loss_func:', self.reg_loss_func)
            # print('@@@@@@@@@@@@@@ MSELoss OK')
        else:
            self.reg_loss_func = F.smooth_l1_loss
    ...
    def get_rp_loss(self, tb_dict=None):
        point_rp_preds = self.forward_ret_dict['point_rp_preds']
        point_rp_labels = self.forward_ret_dict['point_rp_labels']
        rp_loss_all = self.reg_loss_func(point_rp_preds, point_rp_labels)
        # print("@@@@@@@@@@@@ rp_loss_all_size:", rp_loss_all.shape) # [n]

        positives = (point_rp_labels > 0)
        negative_cls_weights = (point_rp_labels == 0) * 1.0
        cls_weights = (negative_cls_weights + 1.0 * positives).float()
        pos_normalizer = positives.sum(dim=0).float()
        cls_weights /= torch.clamp(pos_normalizer, min=1.0)
        # print("@@@@@@@@@@@@ cls_weights_size:", cls_weights.shape)  # [n]

        rp_loss_all = rp_loss_all * cls_weights
        point_rp_loss = rp_loss_all.sum()
        loss_weights_dict = self.model_cfg.LOSS_CONFIG.LOSS_WEIGHTS
        point_rp_loss = point_rp_loss * loss_weights_dict['point_rp_weight']
        # print("@@@@@@@@@@@@ point_rp_loss:", point_rp_loss)

        if tb_dict is None:
            tb_dict = {}
        tb_dict.update({
            'point_rp_loss': point_rp_loss.item()
        })
        return point_rp_loss, tb_dict

    def get_matched_rp_loss(self, tb_dict=None):
        point_rp_preds = self.forward_ret_dict['point_rp_preds']
        point_rp_labels = self.forward_ret_dict['point_rp_labels']
        rp_loss_all = self.reg_loss_func(point_rp_preds, point_rp_labels)
        # print("@@@@@@@@@@@@ rp_loss_all_size:", rp_loss_all.shape) # [n]
        # print("@@@@@@@@@@@@ point_rp_labels_size:", point_rp_labels.shape) # [n]

        # positives = (point_rp_labels > 0)
        # print("@@@@@@@@@@@@ positives_sum1:", positives.sum())  # [n]
        positives = (point_rp_labels >= 0)
        # print("@@@@@@@@@@@@ positives_sum2:", positives.sum())  # [n]
        negative_cls_weights = (point_rp_labels == 0) * 1.0
        cls_weights = (negative_cls_weights + 1.0 * positives).float()
        pos_normalizer = positives.sum(dim=0).float()
        cls_weights /= torch.clamp(pos_normalizer, min=1.0)
