...
class Detector3DTemplate(nn.Module):
    def __init__(self, model_cfg, num_class, dataset):
        ...
        self.module_topology = ['vfe_rp', 'point_head_rp',
    ...
    def build_vfe_rp(self, model_info_dict):
        if self.model_cfg.get('VFE_RP', None) is None:
            return None, model_info_dict

        vferp_module = vfe.__all__[self.model_cfg.VFE_RP.NAME](
            model_cfg=self.model_cfg.VFE_RP,
        ...
        model_info_dict['num_point_features'] = vferp_module.get_output_feature_dim()
        model_info_dict['module_list'].append(vferp_module)
        return vferp_module, model_info_dict

    def build_point_head_rp(self, model_info_dict):
        if self.model_cfg.get('POINT_HEAD_RP', None) is None:
            return None, model_info_dict

        point_head_module = dense_heads.__all__[self.model_cfg.POINT_HEAD_RP.NAME](
            model_cfg=self.model_cfg.POINT_HEAD_RP,
            input_channels=model_info_dict['num_point_features'],
            num_class=self.num_class if not self.model_cfg.POINT_HEAD_RP.CLASS_AGNOSTIC else 1,
