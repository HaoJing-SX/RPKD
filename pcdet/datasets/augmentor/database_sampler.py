...
class DataBaseSampler(object):
    ...
    def add_sampled_boxes_to_scene(self, data_dict, sampled_gt_boxes, total_valid_sampled_dict, mv_height=None, sampled_gt_boxes2d=None):
        ...
        if self.sampler_cfg.get('SECOND_DATA', False):
            # print("@@@@@@@ SECOND_DATA")
            frame_id = int(data_dict['frame_id'])
            # print("@@@@@@@ frame_id:", frame_id, data_dict['frame_id'])
            ...
            for idx, info in enumerate(total_valid_sampled_dict):
                if self.use_shared_memory:
                    ...
                    last_item = os.path.basename(file_path)
                    if self.sampler_cfg.get('MAIN_DATA_MIX', False):
                        mix_id = frame_id // self.sampler_cfg['MIX_DIVISOR']
                        mix_path = self.root_path / self.sampler_cfg['MIX_ITEM'][mix_id] / last_item
                        obj_points = np.fromfile(str(mix_path), dtype=np.float32).reshape(
                    ...
                    # s_points
                    s_file_path = self.root_path / self.sampler_cfg['SECOND_DATA'] / last_item
                    s_obj_points = np.fromfile(str(s_file_path), dtype=np.float32).reshape(
                        [-1, self.sampler_cfg.NUM_POINT_FEATURES])
                    # print("@@@@@@@@@@@@@@@@@ s_file_path:", s_file_path)

                obj_points[:, :3] += info['box3d_lidar'][:3]
                s_obj_points[:, :3] += info['box3d_lidar'][:3]
                if self.sampler_cfg.get('USE_ROAD_PLANE', False):
                    # mv height
                    obj_points[:, 2] -= mv_height[idx]
                    s_obj_points[:, 2] -= mv_height[idx]
                obj_points_list.append(obj_points)
                s_obj_points_list.append(s_obj_points)
            obj_points = np.concatenate(obj_points_list, axis=0)
            s_obj_points = np.concatenate(s_obj_points_list, axis=0)
            ...
            # print("@@@@@@@@@@@@@@@@@ s_points1:", s_points.shape)
            points = box_utils.remove_points_in_boxes3d(points, large_sampled_gt_boxes)
            s_points = box_utils.remove_points_in_boxes3d(s_points, large_sampled_gt_boxes)
            # print("@@@@@@@@@@@@@@@@@ points2:", points.shape)
            # print("@@@@@@@@@@@@@@@@@ s_points2:", s_points.shape)
            points = np.concatenate([obj_points, points], axis=0)
            s_points = np.concatenate([s_obj_points, s_points], axis=0)
            # print("@@@@@@@@@@@@@@@@@ points3:", points.shape)
            # print("@@@@@@@@@@@@@@@@@ s_points3:", s_points.shape)
