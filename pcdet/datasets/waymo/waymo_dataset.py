...
class WaymoDataset(DatasetTemplate):
    ...
    def get_specific_lidar(self, sequence_name, sample_idx, src_name):
        parent_path = os.path.dirname(self.data_path)
        lidar_file = os.path.join(parent_path, src_name, sequence_name, ('%04d.npy' % sample_idx))
    ...
    def __getitem__(self, index):
        ...
        if self.use_shared_memory and index < self.shared_memory_file_limit:
            ...
            if self.dataset_cfg.get('DATA_ASSIGN', False):
                ...
                main_points = self.get_specific_lidar(sequence_name, sample_idx, self.dataset_cfg['DATA_ASSIGN']['MAIN_DATA'])  # compress points
                second_points = self.get_specific_lidar(sequence_name, sample_idx, self.dataset_cfg['DATA_ASSIGN']['SECOND_DATA'])  # ori points
                ...
                input_dict['frame_id'] = info['frame_id']
            else:
                points = self.get_lidar(sequence_name, sample_idx)
                input_dict.update({
                    'points': points,
